...
6. Firestore 安全規則：
   ...
   ```
   > ⚠️ **上面這份規則已經不完整，不要直接照貼。** 2026-09-14 起 §18 書卷旅程加了群組功能，
   > 需要 `groups` 區段；照貼上面這份會讓所有群組操作 permission-denied。
   > **真正的來源是 repo 根目錄的 `firestore.rules`**，貼它、並把裡面的 `ADMIN_UID` 換成自己的 uid
   > （不換的話 `admin.html` 會因為不能列出 users 集合而整個看不到資料）。
- 資料模型：`users/{uid}` 或 `codes/{code}` = `{ planConfig:{order,pace,books,start,cpd,end}, progress:{ [planSig]:[已完成天index] }, ts }`。
  §18 書卷旅程另外在同一份 `users/{uid}` 文件加了 `journeyBooks` / `journeyUpdatedAt` / `myGroups` 三個欄位，
  兩邊都用 `merge:true` 寫入，互不覆蓋。
...
> **2026-09-14 更新：Joseph 決定把它放進共用導覽列，名稱「組隊讀聖經」**（index／year／planner／journey 四頁都有，排在「一年讀經」後面）。下面「刻意不放導覽列」的描述是當時的歷史脈絡。
...
- `journey.html` 提供四種排序：正典／文體分類／編年／易讀優先（字數由短到長）。勾選整卷完成即可，
  沒有章節層級的細粒度追蹤。進度未登入時存 `localStorage["journeyDoneBooks"]`（book id 陣列），
  登入後改存雲端並以 localStorage 當離線備援（見下一段）。
- 「卡關了換一本」＝挑還沒讀的書卷裡 `totalChars` 最小的一本。**有群組時會先過濾成「群組還沒人點亮」的書卷**，
  文案也會變成「你的群組還沒有人讀過這一卷」——這是刻意的機制設計，用來把人推向那鴻書、西番雅書這種沒人碰的冷門小書。
- 成就徽章全部是前端依累積完成度即時計算，沒有排名、沒有互相比較。

### 雲端同步與資料模型（2026-09-14 起，commit `ff81db5`）
沿用 §11 既有的 Firebase 專案與 Google 登入，**沒有另開一套帳號系統**。

```
users/{uid}                      # 與 year.html / planner.html 共用同一份文件
  journeyBooks:     [bookId,…]   # ← §18 專用：已完成的書卷
  journeyUpdatedAt: ts
  myGroups:         [groupId,…]
  progress/profile/…             # ← §11 §17 既有欄位，不可覆蓋

groups/{groupId}                 # groupId 本身就是 8 碼邀請碼
  name, ownerUid, createdAt, memberCount
  coverage: { "b<bookId>": [uid,…] }   # 注意是 uid 陣列，不是計數
groups/{groupId}/members/{uid}
  name, joinedAt
```

- **群組只需要 Google 登入**。§11 的「同步碼」走匿名帳號、uid 會變，不適合當長期群組身分，所以群組不支援同步碼；
  個人進度不受影響。
- 首次登入時把本地與雲端取**聯集**再寫回，兩邊紀錄都不會掉。
- 群組地圖只讀 `groups/{gid}` 一份文件（`coverage` 直接就是答案），不需要逐一查成員，小群組零延遲。

### 三條不可破壞的約定（改這支程式前務必先讀）
1. **寫 `users/{uid}` 一律要 `{merge:true}`。** 這份文件是三個頁面共用的，不 merge 會直接洗掉
   `year.html` 的 `progress` 和 `profile`，而且使用者不會馬上發現。
2. **改 `coverage` 一律用 `update()`，不要用 `set(…,{merge:true})`。** Firestore 的語意是：
   `update()` 的 key 裡的點是**路徑**，`set+merge` 的點是**字面欄位名**。用錯會生出一個真的叫
   `coverage.b32` 的頂層欄位，巢狀結構永遠是空的，而且要到上線才會發現。`tools/test_journey_groups.py`
   有專門一項在擋這個。
3. **`coverage` 存 uid 陣列，不要「優化」成計數。** 用 `arrayUnion`/`arrayRemove` 才有冪等性：
   重複加入不會灌水、退出群組能精準移掉自己那一份而不動到別人。改成 `increment` 計數的話，
   反覆進出群組就會把數字灌爆，且無法還原。

### 雙軌計分為什麼存在（不要「簡化」掉）
原本只用卷數計分，加上「挑最短的一卷」按鈕與短卷徽章，等於系統在教人刷短卷：
讀完 5 卷單章書進度就跳 **7.6%**，而整本以賽亞書 66 章只值 **1.5%**。這是典型的 Goodhart's Law。
現在書卷％與字數％並列顯示，落差直接可見（實測 7.6% vs 0.3%）。
**看到「兩條進度條好像重複、合成一條比較乾淨」的念頭時請忍住**——那個落差就是這個設計的全部重點。
徽章也補了「🐘 啃完一本大部頭」「⚖️ 字數過半」，讓長卷有對應的肯定。

### 群組集體點燈的設計原則
§18 原本的待辦寫過一句警告：加同步／教會端統計前，要先確認這對使用者是加分還是變成壓力。
這一版是在那個前提下設計的，具體做法：
...
- **加入群組會回填自己已讀的書卷**，所以新群組不會是空的、中途才加入的人一進來就有貢獻——
  這同時解掉「空房間」和「太晚加入沒事可做」兩個問題。

### 從一年讀經計畫匯入（刻意做成手動）
`importFromYear()` 讀 `progress.year365` 的已完成天數，展開成章，某卷所有章都完成才標記該卷完成。
推導方向只能是**章 → 卷**：勾了「約拿書」不能反推第 268 天完成，因為那天還含俄巴底亞書。

**刻意做成一次性手動按鈕，沒有做自動推導。** 自動推導會產生一個取消不掉的 bug：使用者手動取消一卷，
下次重新推導又變回完成。要正確支援得同時維護 manual／derived／suppressed 三個集合，不值得。
手動匯入後就是一般紀錄，可自由取消。
...
**改完 `journey.html` 請把這兩份跑過再提交。**

### 書卷導論的版權原則（重要，狀況在 2026-09-13 有變動，先讀完再動 `intro`）
起因是想用大衛鮑森牧師（David Pawson）的逐卷講道當導讀素材。**現況分兩段：**

**第一，素材本身。** `大衛鮑森聖經縱覽/` 底下有他《舊約縱覽》系列的 epub 與轉出的 md，
**是個人使用轉檔，已在 `.gitignore` 排除，絕對不可以 commit 進這個公開 repo。**
有這些檔案不等於有重製授權——他已過世，版權由其事工／遺產管理，「他生前樂意分享」
不等於現在可以任意重製。

**第二，`intro` 欄位。** 2026-09-13（commit `3331171`）用這些逐字稿當**靈感來源**，
重寫了 20 卷的導論（利未記、士師記、路得記、列王紀上下、以斯拉尼希米記、以斯帖記、以西結書、
以賽亞書、但以理書、何西阿書、俄巴底亞約珥書、約伯記、約拿書、阿摩司書、西番雅書、哈巴谷書、哈該書）。
**寫進去的是 100% 原創改寫**——書卷結構、神學重點、歷史背景這類事實性框架，
**沒有照抄他的用詞、他舉的例子、他講的故事**。出埃及記／民數記／申命記／約書亞記維持原本的通用版本。

之後要再動 `intro`，規則不變：
1. 只能改寫成自己的文字，或做簡短引用並註明出處，**不能整段搬運**，也不要重現他獨特的譬喻與軼事。
2. 沒有授權前，網站文案不要宣稱內容出自他，也不要把引號內的話安在他名下。
3. AI 陪讀教練沒有逐字稿（那些檔案不會進 repo、也不該進），
   所以教練那邊仍然只能誠實指向他公開的影片資源，不能假裝知道他講了什麼。
...
**下一個動作（Joseph 要做，不是程式問題）**
1. `git push origin main`。
2. 到 Firebase Console 貼上 `firestore.rules`，**先把 `ADMIN_UID` 換成自己的 uid**（登入 `admin.html` 畫面會顯示）。
   順便確認目前線上的規則是不是還停在很寬鬆的狀態（`admin.html` 能列出所有 users 代表規則相當開放）——
   那是既有狀況、不是這次改動造成的，但值得一起收掉。
3. 真的登入跑一次：建立群組 → 看已讀的書有沒有自動點亮 → 把邀請碼給一個人試加入。
   **這段我沒辦法在開發環境測到**（需要真實 Google 帳號），測試只驗證到資料操作邏輯。
   規則貼錯的症狀是 console 出現 `permission-denied`、畫面上建立群組失敗。

**刻意還沒做的（有想過，結論是現在不做）**
- **熱度地圖**（每卷顯示幾個人讀過）：資料結構已經支援（`coverage` 存的是 uid 陣列，長度就是人數），
  隨時可以改呈現方式。但小樣本下整張圖會淡得讓人沮喪——熱度地圖問「這卷有幾個人讀過」，
  答案幾乎都是 0 或 1；集體點燈問「這卷有沒有人讀過」，15 個人就足以點亮全部。
  等真的有 50-100 人在用再說。
- **分隊／群組層級**（教會底下掛小組）：複雜度直接翻倍，而且會稀釋「我們教會作為一個身體讀完整本聖經」
  的敘事。先用扁平群組跑一陣子，看人實際怎麼用再決定。
- **連續天數（streak）**：Duolingo 式的連續紀錄是所有遊戲化機制裡最強的，但它靠的是損失趨避——
  斷掉會產生罪惡感，而斷了 40 天連續紀錄的人往往直接放棄。跟 §18「不催促、不製造罪惡感」的定位相衝突，
  結論是不做。
- **年度化**：若要把點燈做成每年重來的傳統，注意有期限的活動天生會排擠中途才加入的人
  （十月才想參加，看到 66 格亮了 60 格就沒事可做）。解法是讓已亮的格子能疊加、顏色加深——
  這樣熱度地圖其實會從集體點燈自己長出來，不用另外做一套。

**其他**
- 若拿到鮑森牧師（或其他講員）授權的文字素材，可以另外做一份 `data/book_journey_extra.json` 疊加
  更完整的導論，不要直接覆蓋 `intro` 欄位，保留原創版本當預設／備援。
- ~~`journey.html` 刻意不放進共用導覽列~~ → 2026-09-14 已放進導覽列（「組隊讀聖經」），見本節開頭。
