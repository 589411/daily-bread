    ...
    if(req.method === "POST"){
      ...
      for(const ev of (data.events||[])){
        ...
        if(ev.type === "message" && ev.message && ev.message.type === "text" && ev.replyToken){
          if((ev.message.text||"").includes("每日靈糧")){
        ...
        if(ev.type === "join"){
          ...
          if(ev.replyToken) ctx.waitUntil(linePost(env, "reply", {replyToken:ev.replyToken, messages:[{type:"text", text:"✅ 已加入「每日靈糧」。在群組輸入「每日靈糧」四個字，就會回覆當天的讀經進度。"}]}));
