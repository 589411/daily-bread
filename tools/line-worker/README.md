...
>
> **不回覆的情況**：訊息是網站「分享到 LINE／複製訊息」貼出來的卡片（帶本站網址或 `📖` 開頭的多行卡片）→ 一律安靜。
> 這是為了修 2026-08-04 的 bug：**分享的卡片本身含「每日靈糧」四個字，會勾起 bot 回覆，群組出現兩則一樣的訊息**。
> 關鍵字也加了長度上限（整則 ≤ 9 字才算提問），所以聊天中順口提到「每日靈糧」不會被回。
>
> **測試不必發 LINE**：
> - `https://<worker>/?msg=<整則訊息>` → 跑完整判斷，告訴你會不會回、回什麼（改觸發規則後用這個回歸測）
> - `https://<worker>/?ref=彼前5` → 只預覽單章訊息
> - 本機：`npx wrangler dev --local` 再 `curl --get --data-urlencode "msg=每日靈糧" http://localhost:8787/`
>
